...
sudo nft add rule inet "${WIREGUARD_TABLE_NAME}" INPUT ip saddr "${WIREGUARD_IPv4_SUBNET}" udp dport "${WIREGUARD_DNS_PORT}" ip daddr "${HOST_IPV4}" accept   # This rule accepts DNS queries (UDP on port 53) from clients within the specified IPv4 subnet (10.0.0.0/8) to the specific DNS server (10.0.0.1)
sudo nft add rule inet "${WIREGUARD_TABLE_NAME}" INPUT ip6 saddr "${WIREGUARD_IPv6_SUBNET}" udp dport "${WIREGUARD_DNS_PORT}" ip6 daddr "${HOST_IPV6}" accept # This rule accepts DNS queries (UDP on port 53) from clients within the specified IPv6 subnet (fd00::/8) to the specific DNS server (fd00::1)
sudo nft add rule inet "${WIREGUARD_TABLE_NAME}" INPUT ip saddr "${WIREGUARD_IPv4_SUBNET}" tcp dport "${WIREGUARD_DNS_PORT}" ip daddr "${HOST_IPV4}" accept   # This rule accepts DNS queries (TCP on port 53) from clients within the specified IPv4 subnet (10.0.0.0/8) to the specific DNS server (10.0.0.1)
sudo nft add rule inet "${WIREGUARD_TABLE_NAME}" INPUT ip6 saddr "${WIREGUARD_IPv6_SUBNET}" tcp dport "${WIREGUARD_DNS_PORT}" ip6 daddr "${HOST_IPV6}" accept # This rule accepts DNS queries (TCP on port 53) from clients within the specified IPv6 subnet (fd00::/8) to the specific DNS server (fd00::1)
...
sudo nft add rule inet "${WIREGUARD_TABLE_NAME}" FORWARD ip saddr "${WIREGUARD_IPv4_SUBNET}" udp dport "${WIREGUARD_DNS_PORT}" ip daddr "${HOST_IPV4}" accept   # Allow VPN clients to forward DNS queries to the DNS server
sudo nft add rule inet "${WIREGUARD_TABLE_NAME}" FORWARD ip6 saddr "${WIREGUARD_IPv6_SUBNET}" udp dport "${WIREGUARD_DNS_PORT}" ip6 daddr "${HOST_IPV6}" accept # Allow VPN clients to forward DNS queries to the DNS server
sudo nft add rule inet "${WIREGUARD_TABLE_NAME}" FORWARD ip saddr "${WIREGUARD_IPv4_SUBNET}" tcp dport "${WIREGUARD_DNS_PORT}" ip daddr "${HOST_IPV4}" accept   # Allow VPN clients to forward DNS queries (TCP) to the DNS server
sudo nft add rule inet "${WIREGUARD_TABLE_NAME}" FORWARD ip6 saddr "${WIREGUARD_IPv6_SUBNET}" tcp dport "${WIREGUARD_DNS_PORT}" ip6 daddr "${HOST_IPV6}" accept # Allow VPN clients to forward DNS queries (TCP) to the DNS server
# --- FORWARD CHAIN (Filtering forwarded traffic) ---
...
# This test checks for syntax errors and displays the nftables rules that would be applied

...
sudo nft add chain inet "${WIREGUARD_TABLE_NAME}" PREROUTING { type nat hook prerouting priority dstnat \; policy accept \; } # NAT PREROUTING chain processes incoming packets before routing, typically for destination NAT (e.g., forwarding requests)
...
sudo nft add chain inet "${WIREGUARD_TABLE_NAME}" POSTROUTING { type nat hook postrouting priority srcnat \; policy accept \; }             # POSTROUTING chain handles packets after routing, used for source NAT (masquerading)
